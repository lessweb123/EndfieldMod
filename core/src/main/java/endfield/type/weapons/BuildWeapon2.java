...
/** @author DdJjFf */
public class BuildWeapon2 extends Weapon {
	...
	public BuildWeapon2(String name) {
		super(name);
		rotate = true;
		noAttack = true;
		predictTarget = false;
	...
	public void update(Unit unit, WeaponMount mount) {
		...
		if (unit.controller() instanceof CommandAI commandAI) {
			if ((commandAI.command == UnitCommand.assistCommand ||
					...
					UnitCommands2.commandControllerAccessor.getObject(commandAI) instanceof BuilderAI builderAI) {
	...
	public BuildPlan findPlan(Unit unit, Unit weaponUnit, BuildWeaponMount tm) {
		Queue<BuildPlan> plans = unit.plans();
		return plans.find(p -> {
			if (unit.plans.first() == p || !weaponUnit.within(p.x * 8, p.y * 8, Vars.state.rules.infiniteResources ? 2000f + range : range)) return false;
	...
	public void check(Unit unit, BuildWeaponMount m) {
	...
	public void findTarget(Unit unit, Unit weaponUnit, BuildWeaponMount m) {
	...
	public boolean isRob(Unit unit, BuildWeaponMount m) {
	...
	public void drawBuildingBeam(float px, float py, BuildWeaponMount mount, Unit unit) {
		...
		if (tile != null && Mathf.dst(plan.drawx(), plan.drawy(), px, py) <= (Vars.state.rules.infiniteResources ? Float.MAX_VALUE : range)) {
			...
			Drawf.buildBeam(px, py, tx, ty, (float) (8 * size) / 2f);
