...
public class OverdriveWeapon extends Weapon {
	...
	public float extraDuration = 120f;
	public float beamSmoothness = 0.1f;
	...
	public void addStats(UnitType u, Table t) {
		t.row();
		t.add("[lightgray]" + Stats2.overdrive.localized() + ": " + (mirror ? "2x " : "") + "[white]" + (int) (speedBoost * 100) + " " + StatUnit.percent.localized());
	}
	...
	protected Teamc findTarget(Unit unit, float x, float y, float range, boolean air, boolean ground) {
		return Units.findAllyTile(unit.team, x, y, range, b -> b.block.canOverdrive);
	}
	...
	public void update(Unit unit, WeaponMount m) {
		...
		if (!(m instanceof OverdriveWeaponMount mount)) return;
		...
		if (canShoot) {
			...
			Building build = Vars.world.build(World.toTile(mount.lastEnd.x), World.toTile(mount.lastEnd.y));
	...
	public static class OverdriveWeaponMount extends WeaponMount {
		...
		public Vec2 lastEnd = new Vec2(), pointPos = new Vec2(), realLastEnd = new Vec2();
