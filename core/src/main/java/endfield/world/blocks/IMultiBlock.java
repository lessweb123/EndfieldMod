 ...
 * @author LaoHuaJi
 */
public interface IMultiBlock extends IBlock {
	Seq<Point2> linkPos();
	...
	default Seq<Building> setLinkBuild(Building build, Block block, Tile tile, Team team, int size, int rotation) {
		...
		for (int i = 0; i < linkPos.size; i++) {
			...
			if (!block.outputsLiquid) {
				t.setBlock(Blocks2.linkBlock[s - 1], team, 0);
			} else {
				t.setBlock(Blocks2.linkBlockLiquid[s - 1], team, 0);
			}
			((LinkBlock.LinkBuild) t.build).updateLink(build);
			out.add(t.build);
		}
		return out;
	}
