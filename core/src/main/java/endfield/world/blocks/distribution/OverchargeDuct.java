...
public class OverchargeDuct extends Duct {
	...
	public OverchargeDuct(String name) {
		super(name);
	...
	public class OverchargeDuctBuild extends DuctBuild {
		...
		protected void drawAtWithGlow(float x, float y, int bits, float rotation, SliceMode slice) {
			...
			if (sliced(glowRegions[bits], slice).found() && power != null && power.status > 0f && slice.equals(SliceMode.none)) {
