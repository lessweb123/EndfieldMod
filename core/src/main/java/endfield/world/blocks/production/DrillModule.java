...
import endfield.world.blocks.production.ModuleDrill.ModuleDrillBuild;
...
public class DrillModule extends Block {
	...
	public Seq<Item[]> convertList = new Seq<>(Item[].class);
	...
	public void load() {
		...
		topRegion = Sprites.splitArray(Core.atlas.find(name + "-top"), 64, 1);
	}
	...
	public void setStats() {
		...
		if (powerMul != 0 || powerExtra != 0)
			stats.add(Stats2.powerConsModifier, Core.bundle.get("nh.stat.power-cons-modifier"), Strings.autoFixed(powerMul * 100, 0), Strings.autoFixed(powerExtra, 0));
		if (boostSpeed != 0 || boostFinalMul != 0)
			stats.add(Stats2.minerBoosModifier, Core.bundle.get("nh.stat.miner-boost-modifier"), Strings.autoFixed(boostSpeed * 100, 0), Strings.autoFixed(boostFinalMul * 100, 0));
		if (convertList.size > 0) stats.add(Stats2.itemConvertList, getConvertList());
	}
	...
	public void drawer(DrillModuleBuild drill) {}

	public class DrillModuleBuild extends Building {
		public @Nullable ModuleDrillBuild drillBuild;
		...
		public boolean canApply(ModuleDrillBuild drill) {
		...
		public void apply(ModuleDrillBuild drill) {}
	}
}
