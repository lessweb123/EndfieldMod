...
public class MechPad extends Block {
	...
	public static void playerSpawn(Tile tile, Player player) {
		if (player == null || tile == null || !(tile.build instanceof MechPadBuild)) return;
	...
	public void init() {
		...
		for (Consume c : consumeBuilder) c.multiplier = b -> Vars.state.rules.unitCost(b.team);
	...
	public class MechPadBuild extends Building implements IControlBlock {
		...
		public void updateTile() {
			...
			if (progress >= buildTime && isControlled()) {
				...
				spawnMech(unitType, tile, player);
			}
		}
