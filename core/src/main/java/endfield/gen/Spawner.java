...
public class Spawner extends BaseEntity implements Syncc, Timedc, IScaled, Rotc {
	...
	public void add() {
		...
		Fx2.spawnWave.at(x, y, drawSize * 1.1f, team.color);
	...
	public void effect() {
		...
		Sounds2.jumpIn.at(toSpawn.x, toSpawn.y);
		if (type.flying) {
			Fx2.jumpTrail.at(toSpawn.x, toSpawn.y, rotation(), team.color, type);
			toSpawn.apply(StatusEffects.slow, Fx2.jumpTrail.lifetime);
		} else {
			Fx2.spawn.at(x, y, type.hitSize, team.color);
			Fx.unitSpawn.at(toSpawn.x, toSpawn.y, rotation(), type);
			Time.run(Fx.unitSpawn.lifetime, () -> {
				for (int j = 0; j < 3; j++) {
					Time.run(j * 8, () -> Fx.spawn.at(toSpawn));
				}
				Fx2.spawnGround.at(toSpawn.x, toSpawn.y, type.hitSize / Vars.tilesize * 3, team.color);
				Fx2.circle.at(toSpawn.x, toSpawn.y, type.hitSize * 4, team.color);
			});
		}
