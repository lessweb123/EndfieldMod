	...
	public void update(Unit unit) {
		...
		if ((timer += Time.delta) >= reload) {
			...
			for (int i = 0; i < len; i++) {
				...
				Building absorber = Damage.findAbsorber(unit.team, rx, ry, other.getX(), other.getY());
				if (absorber != null) {
					other = absorber;
				}

				if (other instanceof Teamc teamc && teamc.team() == unit.team) return;

				anyNearby = true;
				if (other instanceof Building b) {
					b.damage(unit.team, damage * Vars.state.rules.unitDamage(unit.team));
				} else {
					other.damage(damage * Vars.state.rules.unitDamage(unit.team));
				}
