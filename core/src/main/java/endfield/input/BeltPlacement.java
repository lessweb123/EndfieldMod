...
public final class BeltPlacement {
	static final Seq<BuildPlan> plans1 = new Seq<>(BuildPlan.class);
	static final Seq<Point2> tmpPoints = new Seq<>(Point2.class), tmpPoints2 = new Seq<>(Point2.class);
		...
		for (int i = 0; i < plans.size; ) {
			...
			if (i < plans.size - 1 && placeable.get(cur) && (!placeable.get(plans.get(i + 1)))) {
				...
				for (int j = i + 2; j < plans.size; j++) {
					...
					if ((bridge instanceof DirectionBridge dBridge && !dBridge.positionsValid(cur.x, cur.y, other.x, other.y))) {
